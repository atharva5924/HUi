...
function Navbar(props: { isUser?: boolean; isVerifier?: boolean }) {
    ...
    <div className="h-16 w-full relative">
      <nav className="grid grid-cols-[7fr_3fr] h-full w-full bg-white shadow-md">
        <div className="flex items-center h-full w-full pl-4 font-extrabold text-green-700">
          <p className="flex items-center md:w-3/6 h-full w-2/6 text-sm md:text-2xl">
            CREDIT APP
          </p>
          <div className="w-24 absolute lg:hidden md:ml-40 ml-24">
            {!props.isUser && (
              <img
                className="md:h-auto md:w-auto h-5 w-5 cursor-pointer"
                src={DropMenu}
                onClick={handleMenu}
              ></img>
            )}
          ...
          {props.isUser && (
            <div className="flex items-center justify-evenly w-5/6 h-full">
              <img
                className="md:h-auto md:w-auto h-5 w-5 cursor-pointer"
                src={Home}
              ></img>
          ...
          <button onClick={() => handleLogout(navigate)}>
            <img className=" h-8 w-12 cursor-pointer" src={Image}></img>
          </button>
        </div>
        <div className="flex items-center justify-evenly h-full w-full font-bold text-green-700">
          <img
            className="h-5 w-5 md:h-auto md:w-auto cursor-pointer"
            src={Notification}
          ></img>
          <img
            className="h-5 w-5 md:h-auto md:w-auto cursor-pointer"
